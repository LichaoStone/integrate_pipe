...
public class WebSocketInterceptor implements HandshakeInterceptor {
	...
	public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response, WebSocketHandler wsHandler, Map<String, Object> attributes) throws Exception {
		...
		if (request instanceof ServletServerHttpRequest) {
        	...
        	//外采调度（视频通话专用）
        	String webCallFlag=req.getParameter("webCallFlag");
        	...
        	if(StringUtils.isEmpty(pageFlag) || StringUtils.isEmpty(actionFlag) ){
        		...
        		logger.info("webscoket连接请求，页面标志pageFlag："+pageFlag+",动作标志："+actionFlag+",webCallFlag:"+webCallFlag);
        		// 将页面标识和动作标识放入参数中，之后的session将根据这两个值来区分
        		attributes.put("pageFlag", pageFlag.trim());
        		attributes.put("actionFlag",actionFlag.trim());
        		attributes.put("webCallFlag",webCallFlag);
        	}
        } else {
        	flag = false;
        }
